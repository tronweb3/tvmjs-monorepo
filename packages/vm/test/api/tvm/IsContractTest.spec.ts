...
contract isTestCtr {
  ...
  */
  it('testIsContract', async () => {
    const abi = JSON.parse(
      '[{"constant":true,"inputs":[],"name":"senderIsContrct","outputs"' +
    ...
    const bytecode =
      '608060405234801561001057600080fd5b50d3801561001d57600080fd5b50d28015610' +
    ...
    const contractAddressOther = (await deployContract(
      vm,
      {
        caller,
        bytecode: hexToBytes(`0x${bytecode}`),
    ...
    const abiItem1 = abi.find((item: any) => item.name === 'isTest')
    const result1 = await triggerConstant(vm, {
      caller,
      abi: abiItem1,
      contractAddress,
      params: [utils.address.toHex('27k66nycZATHzBasFT9782nTsYWqVtxdtAc').replace(/^a0/, '0x')],
    })
    ...
    const abiItem3 = abi.find((item: any) => item.name === 'isContrct')
    ...
    const abiItem4 = abi.find((item: any) => item.name === 'senderIsContrct')
    const result4 = await triggerConstant(vm, {
      caller,
      abi: abiItem4,
      contractAddress,
    })
    assert.equal(result4[0], false)

    await trigger(vm, {
      caller,
      contractAddress,
      abi: abi.find((item: any) => item.name === 'isTestSender'),
    })

    const abiItem5 = abi.find((item: any) => item.name === 'senderIsContrct')
    const result5 = await triggerConstant(vm, {
      caller,
      abi: abiItem5,
      contractAddress,
    })
    assert.equal(result5[0], false)

    await trigger(vm, {
      caller,
      contractAddress,
      abi: abi.find((item: any) => item.name === 'withCall'),
      params: [contractAddressOther.toString()],
    })

    const abiItem6 = abi.find((item: any) => item.name === 'senderIsContrct')
    const result6 = await triggerConstant(vm, {
      caller,
      abi: abiItem6,
      contractAddress: contractAddressOther,
    })
    ...
    await trigger(vm, {
      caller,
      contractAddress,
      abi: abi.find((item: any) => item.name === 'withDelegatecall'),
      params: [contractAddressOther.toString()],
    })

    const abiItem7 = abi.find((item: any) => item.name === 'senderIsContrct')
    const result7 = await triggerConstant(vm, {
      caller,
      abi: abiItem7,
      contractAddress,
    })
    assert.equal(result7[0], false)
